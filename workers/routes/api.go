...
func SetupRoutes(repos *db.Repositories, driverService *drivers.DriverService) *http.ServeMux {
	...
	// 文件操作路由
	mux.HandleFunc("/api/fs/list", corsMiddleware(requireAuth(driverHandler.ListFiles, repos)))
	mux.HandleFunc("/api/fs/get", corsMiddleware(requireAuth(driverHandler.GetFile, repos)))
	
	// 文件下载路由
	mux.HandleFunc("/d/", corsMiddleware(driverHandler.GetFileLink))
	mux.HandleFunc("/download/", corsMiddleware(driverHandler.DownloadFile))
